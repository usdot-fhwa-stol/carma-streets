...
{
    intersection_model::intersection_model *model = new intersection_model::intersection_model("../vector_map.osm");
...
TEST(intersection_model_test, read_json_file)
{
    intersection_model::intersection_model model("../vector_map.osm");
    rapidjson::Document doc;
    doc = model.read_json_file("../manifest.json");
    ASSERT_FALSE(doc.HasParseError());
    ASSERT_TRUE(doc["osm_file_path"].IsString());
    try
    {
        doc = model.read_json_file("../manifest_bad_path.json");
    }
    catch (...)
    {
...
TEST(intersection_model_test, initialization)
{
    // Inside the default constructor, the initialize() method is called to initialize the intersection information.
    intersection_model::intersection_model model;
    ASSERT_NE(model.get_intersection_info().intersection_id, 0);
    ASSERT_NE(model.get_intersection_info().intersection_name.size(), 0);
    ASSERT_EQ(model.get_intersection_info().link_lanelets.size(), 12);
    ASSERT_EQ(model.get_intersection_info().entering_lanelets.size(), 4);
    ASSERT_EQ(model.get_intersection_info().departure_lanelets.size(), 4);
}