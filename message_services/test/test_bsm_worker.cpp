...
TEST(test_bsm_worker, pop_cur_element_from_list)
{
    ...
    ASSERT_EQ("bsmid3", bsm_w_obj.get_curr_list().at(1).getCore_data().temprary_id);
    ASSERT_EQ(38.956287, bsm_w_obj.get_curr_list().at(1).getCore_data().latitude);
    ASSERT_EQ(72, bsm_w_obj.get_curr_list().at(1).getCore_data().elev);
    ASSERT_EQ(-77.150492, bsm_w_obj.get_curr_list().at(1).getCore_data().longitude);
    ASSERT_EQ(13, bsm_w_obj.get_curr_list().at(1).getCore_data().speed);
    ASSERT_EQ(12, bsm_w_obj.get_curr_list().at(1).getCore_data().size.length);
    ASSERT_EQ(16323, bsm_w_obj.get_curr_list().at(1).getCore_data().sec_mark);
    ASSERT_EQ(12, bsm_w_obj.get_curr_list().at(1).getCore_data().msg_count);
}