...
def store_kafka_topic(container_name ,topic, dir, timeout, start_time, end_time):
    timestamp_regex = 'CreateTime:(\d+)\t{'
    filename = f'{dir}/{topic}_{container_name}.log'
    ...
    command = f'docker exec {container_name} timeout {timeout} kafka-console-consumer.sh --topic {topic} --property print.timestamp=true --from-beginning --bootstrap-server localhost:9092'
    result = subprocess.check_output(f'docker exec {container_name} kafka-run-class.sh kafka.tools.GetOffsetShell --broker-list localhost:9092 --topic {topic} | awk -F \":\" \'{{sum += $3}} END {{print sum}}\'', shell=True)
    if result == b'\n':
        return 1
    else:
        num_msgs = int(result)
...
def main():
    ...
    for topic in topics:
        for container_name in args.container_names:
            ret = store_kafka_topic(container_name, topic, outdir, timeout, start_time, end_time)
            if ret != 0:
