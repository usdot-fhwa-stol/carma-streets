...
void vehicle::update(const rapidjson::Document& message, intersection_client& localmap){
	...
	*/
	...
	// Read configs
	auto stop_distance = streets_service::streets_configuration::get_double_config("stop_distance");
	auto stop_speed = streets_service::streets_configuration::get_double_config("stop_speed");
	auto max_valid_speed = streets_service::streets_configuration::get_int_config("max_valid_speed");
	...
	if (message["metadata"].HasMember("timestamp") && (double)message["metadata"]["timestamp"].GetInt64() / 1000.0 >= timestamp){
		...
		*/
		if (message["payload"].HasMember("cur_speed")){
			if (message["payload"]["cur_speed"].GetDouble() * 0.02 > max_valid_speed && id != "" && message["payload"].HasMember("cur_ds") && message["payload"].HasMember("cur_lane_id")){
				...
				SPDLOG_CRITICAL("Invalid speed in the message received from {0}: {1}", veh_id, invalid_speed);
			}
			else{
				speed = message["payload"]["cur_speed"].GetDouble() * 0.02;
			}
		} else{
			SPDLOG_CRITICAL("the current speed of Vehicle {0} is missing in the received update!", veh_id);
		}

		if (message["payload"].HasMember("cur_accel")){
			acceleration = message["payload"]["cur_accel"].GetDouble();
		} else{
			SPDLOG_CRITICAL("the current acceleration of Vehicle {0} is missing in the received update!", veh_id);
		}
		...
		*/
		...
		if (message["payload"].HasMember("cur_lane_id")){
			...
			SPDLOG_CRITICAL("the current lane id of Vehicle {0} is missing in the received update!", veh_id);
		}

		if (message["payload"].HasMember("cur_ds")){
			distance = message["payload"]["cur_ds"].GetDouble();
		} else{
			SPDLOG_CRITICAL("the current destination of Vehicle {0} from the end of the lane is missing in the received update!", veh_id);
		}
			
		if (message["payload"].HasMember("is_allowed")){
			access = message["payload"]["is_allowed"].GetBool();
		} else{
			SPDLOG_CRITICAL("the access status of Vehicle {0} is missing in the received update!", veh_id);
		}
		...
		if (message["payload"].HasMember("entry_lane_id") && message["payload"]["entry_lane_id"].GetInt() != 0){
			...
			SPDLOG_CRITICAL("the entry lane id of Vehicle {0} is missing in the received update!", veh_id);
		}
		...
		if (message["payload"].HasMember("dest_lane_id") && message["payload"]["dest_lane_id"].GetInt() != 0){
			...
			SPDLOG_CRITICAL("the exit lane id of Vehicle {0} is missing in the received update!", veh_id);
		}
		...
		if (message["payload"].HasMember("link_lane_id") && message["payload"]["link_lane_id"].GetInt() != 0){
			...
			SPDLOG_CRITICAL("the link lane id of Vehicle {0} is missing in the received update!", veh_id);
		}
		...
		if (id == ""){
			...
			*/
			if (message["payload"].HasMember("v_length")){
				...
				SPDLOG_CRITICAL("the length of Vehicle {0} is missing in the received update!", veh_id);
			}
			
			if (message["payload"].HasMember("min_gap")){
				min_gap = message["payload"]["min_gap"].GetDouble();
			} else{
				SPDLOG_CRITICAL("the min gap of Vehicle {0} is missing in the received update!", veh_id);
			}
			
			if (message["payload"].HasMember("react_t")){
				reaction_time = message["payload"]["react_t"].GetDouble();	// !!!
			} else{
				SPDLOG_CRITICAL("the reaction time of Vehicle {0} is missing in the received update!", veh_id);
			}
			
			if (message["payload"].HasMember("max_accel")){
				accel_max = message["payload"]["max_accel"].GetDouble();
			} else{
				SPDLOG_CRITICAL("the maximum acceleration of Vehicle {0} is missing in the received update!", veh_id);
			}

			if (message["payload"].HasMember("max_decel")){
				decel_max = message["payload"]["max_decel"].GetDouble();
			} else{
				SPDLOG_CRITICAL("the maximum decelration of Vehicle {0} is missing in the received update!", veh_id);
			}

			if (message["payload"].HasMember("direction")){
				direction = message["payload"]["direction"].GetString();
			} else{
				SPDLOG_CRITICAL("the direction of Vehicle {0} at the intersection box is missing in the received update!", veh_id);
			}
			...
			if (lane_id == entryLane_id){
				if (distance <= stop_distance + length && speed <= stop_speed){
					state = "RDV";
					st_actual = timestamp;
				} else{
					state = "EV";
			...
			if (state == "EV"){
				if (lane_id != entryLane_id){
					SPDLOG_CRITICAL("Vehicle Class Issue - The lane ID of vehicle {0} is not correct! Vehicle {0} is in EV state, but its lane ID in the status and intent message is a link ID; lane_id: {1}, entry_lane_id: {2}, link_id: {3}", veh_id, lane_id, entryLane_id, link_id);
				}
				else if (distance <= stop_distance + length && speed <= stop_speed){
					state = "RDV";
					st_actual = timestamp;
				}
				access = false;
			...
			for (SizeType i = 0; i < message["payload"]["est_paths"].Size(); ++i){
				...
				if (message["payload"]["est_paths"][i].HasMember("ts") && message["payload"]["est_paths"][i].HasMember("id") && message["payload"]["est_paths"][i].HasMember("ds")){
					...
					if (message["payload"]["est_paths"][i]["id"].GetInt() == 0){
						SPDLOG_CRITICAL("the lane id in the future path of Vehicle {0} is invalid in the received update!", veh_id);
					}
					else if (message["payload"]["est_paths"][i]["ds"].GetDouble() < 0){
						SPDLOG_CRITICAL("the distance to the end of lane in the future path of Vehicle {0} is invalid in the received update!", veh_id);
					}
					else{
						...
						* */
						if (future_info[future_info.size() - 1].distance >= message["payload"]["est_paths"][i]["ds"].GetDouble()){
							...
							SPDLOG_DEBUG("future path {0}: {1}, {2}, {3}, {4}", i, fi.lane_id, fi.distance, fi.speed, fi.acceleration);
							future_info.push_back(fi);
						} 
						else{
							SPDLOG_INFO("Vehicle Class log - vehicle {0}: the lane id of the vehicle in the est_pathh has changed!", veh_id);
							break;
						}
					...
					SPDLOG_CRITICAL("a point in the future path of Vehicle {0} is not complete in the received update!", veh_id);
				}
			}
		}
		else{
			SPDLOG_CRITICAL("the future paths of Vehicle {0} is missing in the received update!", veh_id);
		}

		SPDLOG_INFO("Vehicle Class Vehicle Info Update - timestamp = {0}, vehicle = {1}, lane_id = {2}, state = {3}, speed = {4} m/s, distance = {5} m, access = {6}, departure position index = {7}", timestamp, id, lane_id, state, speed, distance, access, departurePosition_index);
...
bool vehicle::message_hasError(const Document& message, intersection_client& localmap){
	...
	/* if it is the first time processing the vehicle update, the entry_lane_id and link_lane_id must be valid. */
		...
		if (!message["payload"].HasMember("entry_lane_id") || message["payload"]["entry_lane_id"].GetInt() == 0){
			...
			SPDLOG_CRITICAL("Vehicle {0} is not in the communication area yet!", veh_id);
			return true;
		}
		else{
			return false;
	...
	/* if a vehicle update has been succesfully processed before, the vehicle cur_lane_id must be the same as entry_lane_id, link_lane_id, or depart_lane_id . */
		...
		if (to_string(message["payload"]["cur_lane_id"].GetInt()) != entryLane_id && to_string(message["payload"]["cur_lane_id"].GetInt()) != link_id && to_string(message["payload"]["cur_lane_id"].GetInt()) != exitLane_id){
			...
			SPDLOG_CRITICAL("the current lane id of Vehicle {0} is not correct! entry_lane_id: {1}, link_lane_id: {2}, exit_lane_id: {3}, cur_lane_id: {4}", veh_id, entryLane_id, link_id, exitLane_id, to_string(message["payload"]["cur_lane_id"].GetInt()));
			return true;
		}
		else{
			return false;
