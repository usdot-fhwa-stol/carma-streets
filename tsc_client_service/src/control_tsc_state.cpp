...
namespace traffic_signal_controller_service
{
    ...
    void control_tsc_state::run_clear_all_snmp_commands() const
    {
        streets_snmp_cmd::streets_snmp_cmd_converter converter;
        auto snmp_cmds = converter.create_clear_all_snmp_commands(0);//Set default execution time 0 because it is executed immediately at following line
        for(auto& snmp_cmd: snmp_cmds)
        {
            run_snmp_cmd_set_request(snmp_cmd);
        }
    }
}