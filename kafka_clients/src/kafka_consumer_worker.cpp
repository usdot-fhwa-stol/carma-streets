...
namespace kafka_clients
{
    ...
    kafka_consumer_worker::kafka_consumer_worker(const std::string &broker_str, const std::string &topic_str, 
                                                std::string &group_id_str, int32_t partition, int64_t cur_offset)
                                                :_broker_str(broker_str),_topics_str(topic_str),_group_id_str(group_id_str),
                                                _partition(partition),_cur_offet(cur_offset)
    {
        _last_offset = 0;
        _consumer = NULL;
        _topic = NULL;
        _cur_offet = RdKafka::Topic::OFFSET_BEGINNING;
        _partition = 0;
        _run =false;
    }
    ...
    const char* kafka_consumer_worker::msg_consume(RdKafka::Message* message, void* opaque)
    {
        ...
        case RdKafka::ERR_NO_ERROR:
            ...
            std::cout<< _consumer->name()<<" Message Consumed: ( "<<static_cast<int>(message->len())<< " bytes ):  " << static_cast<const char*>(message->payload())<<std::endl;
